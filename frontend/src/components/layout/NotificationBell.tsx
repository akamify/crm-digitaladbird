  ...
  useEffect(() => {
    ...
    connectSocket().then((socket) => {
      const handler = (notification?: UserNotification) => {
        ...
        const belongsToUser = !notification?.user_id || notification.user_id === user?.id;
        if (belongsToUser && notification?.title) {
          playNotificationSound().then(result => {
            if (result.reason === 'blocked') {
              toast('Click Enable Sound to allow notification audio.', { id: 'notification-sound-blocked' });
            }
          });
          showBrowserNotification(notification.title, notification.body || notification.message || null);
        }
