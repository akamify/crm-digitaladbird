...
export function LeadAnalyticsPeriodControl({ scope, onChange, showMode = true }: Props) {
  ...
  function changeMode(view: LeadViewMode) {
    onChange(view === 'all_time' ? { view, from: null, to: null } : presetRange('today'));
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      {showMode && (
        <div className="flex items-center rounded-xl border border-blue-100 bg-white/90 p-1 shadow-sm">
          {([['all_time', 'All Time Leads'], ['daily', 'Daily Lead View']] as Array<[LeadViewMode, string]>).map(([view, label]) => (
            <button key={view} type="button" onClick={() => changeMode(view)} className={clsx(
              'rounded-lg px-3 py-2 text-xs font-semibold transition',
              scope.view === view ? 'bg-brand-600 text-white shadow-sm' : 'text-slate-600 hover:bg-blue-50',
            )}>{label}</button>
          ))}
        </div>
      )}
      ...
      {scope.view === 'daily' ? (
        <div ref={rootRef} className="relative flex items-center rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
          ...
          <button type="button" onClick={() => setOpen(value => !value)} className="inline-flex min-w-40 items-center justify-center gap-2 rounded-lg px-2 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50">
            <span>{formatAnalyticsPeriod(scope)}</span><CalendarDays className="h-4 w-4 text-brand-600" />
          </button>
          ...
          {open && (
            <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-[min(92vw,420px)] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-300/50">
              <div className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Select analytics period</div>
      ...
      ) : <div className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-medium text-slate-500 shadow-sm">Complete CRM history</div>}
    </div>
  );
}
