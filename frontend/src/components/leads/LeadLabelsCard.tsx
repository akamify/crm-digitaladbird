...
export function LeadLabelsCard({ leadId, canManage, createSignal = 0 }: { leadId: string; canManage: boolean; createSignal?: number }) {
  ...
  useEffect(() => {
    if (createSignal > 0 && canManage) setManageOpen(true);
    ...
    <section className="card-padded">
      ...
      {assigned.isLoading ? <Skeleton className="h-10" /> : (
        <div className="flex flex-wrap gap-2">
          ...
          {(assigned.data || []).map(label => (
            <span key={label.id} className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium text-white" style={{ backgroundColor: label.color }} title={label.visibility === 'custom' ? `Created by ${label.created_by_name || 'user'}` : 'Admin label'}>
              {label.name}
              {canManage && (
                <button type="button" onClick={() => remove.mutate({ leadId, labelId: label.id }, { onError: () => toast.error('Could not remove label') })} className="rounded-full p-0.5 hover:bg-black/15" aria-label={`Remove ${label.name}`}>
                  <X className="h-3 w-3" />
                </button>
              )}
            </span>
      ...
      {canManage && (
        <div className="mt-3 flex gap-2">
          <select value={selectedLabelId} onChange={event => setSelectedLabelId(event.target.value)} className="input min-w-0 flex-1 text-xs">
            <option value="">Assign a label</option>
            {available.map(label => <option key={label.id} value={label.id}>{label.name}{label.visibility === 'custom' ? ' (custom)' : ''}</option>)}
          </select>
          <button type="button" onClick={assignSelected} disabled={!selectedLabelId || assign.isPending} className="btn-outline rounded-lg px-3 py-2 text-xs">Assign</button>
          <button type="button" onClick={() => setManageOpen(true)} className="btn-outline inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs">
    ...
    >
      <div className="space-y-4">
        <label className="block">
          ...
          <input value={name} maxLength={60} onChange={event => setName(event.target.value)} className="input w-full" placeholder="e.g. High Intent" />
        </label>
        <div>
