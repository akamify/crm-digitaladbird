...
export default function LeadsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[Leads Error]', error); }, [error]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-xl">
        ...
        <h2 className="text-xl font-bold text-slate-800 mb-2">Leads page error</h2>
        <p className="text-sm text-slate-500 mb-6">Something went wrong loading leads. This is usually temporary.</p>
