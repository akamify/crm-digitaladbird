...
async function sendDirectNotificationEmail({ email, fullName = null, userId = null, emailType, subject, html, text, metadata = {} }) {
  ...
  try {
    ...
    logger.warn({ err: err.message, email, emailType }, '[NotificationEmail] direct email log create failed');
  }
  ...
  try {
    ...
    logger.warn({ email, emailType, provider, code: error.code || 'EMAIL_SEND_FAILED' }, '[NotificationEmail] direct send failed');
    return { status: 'failed', error: message };
  }
}

module.exports = { sendNotificationEmail, sendDirectNotificationEmail };
